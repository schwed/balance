...
class OrientGraphDSL {
     ...
     * @return
     */
    static <T> T withTransaction(OrientGraph orientGraph,
                                 @ClosureParams(value = FromString, options = 'com.tinkerpop.blueprints.impls.orient.OrientGraph') Closure<T> closure) {
        try {
            orientGraph.begin()
