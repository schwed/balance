package com.kon.balance.data.domain.orient.ast

import com.kon.balance.data.domain.orient.ast.util.ASTUtil
import com.kon.balance.data.domain.orient.ast.util.EntityStructure
import com.kon.balance.data.domain.orient.document.OrientDocument
...
import com.kon.balance.data.domain.orient.graph.OrientGraphHelper
import com.kon.balance.data.domain.orient.graph.Vertex
import com.kon.balance.data.domain.orient.schema.SchemaHelper
...
class OrientStructure extends EntityStructure<OrientProperty> {
    ...
    def createEdgeSetter(OrientProperty property) {
        Statement setterStatement = null
        ...
        def outNode = (ClassNode) ASTUtil.annotationValue(edgeNodeAnnotation.members.to)
        if (outNode == entity) {
            setterStatement = returnS(callGraphHelper('createEdge', args(varX(injectedObject), propX(setterVar, 'vertex'), edgeClass)))
        }
        if (inNode == entity) {
            setterStatement = returnS(callGraphHelper('createEdge', args(propX(setterVar, 'vertex'), varX(injectedObject), edgeClass)))
        }
        def methodName = "addTo${property.nodeName.capitalize()}"
        entity.addMethod(new MethodNode(methodName, ACC_PUBLIC, edgeClass.type.plainNodeReference, params(parameter), [] as ClassNode[], setterStatement))
    }
